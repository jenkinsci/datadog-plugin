...
1. Optionally set the `GITHUB_USERNAME` and `GITHUB_PERSONAL_ACCESS_TOKEN` environment variables with the name of your GitHub user and a personal access token value. These variables will be used to create credentials for GitHub Branch Source plugin, which allows to automatically discover GitHub pull-requests from authorised repos and build them in Jenkins. You can use [this link](https://github.com/settings/tokens/new?scopes=repo,public_repo,admin:repo_hook&description=Jenkins+Plugin+Local+Testing) to generate a token (it needs to be a classic token with `repo` and `admin:repo_hook` permissions enabled)   
